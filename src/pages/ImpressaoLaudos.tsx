...
const ImpressaoLaudos = () => {
  ...
  const handleFilter = () => {
    ...
    if (filters.requisicao) {
      filtered = filtered.filter(r => r.numero.toLowerCase().includes(filters.requisicao.toLowerCase()));
    }
    if (filters.paciente) {
      filtered = filtered.filter(r => r.paciente.toLowerCase().includes(filters.paciente.toLowerCase()));
    }
    if (filters.data) {
      filtered = filtered.filter(r => r.dataAtendimento === filters.data);
    }
    if (filters.convenio) {
      filtered = filtered.filter(r => r.convenio.toLowerCase().includes(filters.convenio.toLowerCase()));
    }
    if (filters.situacao) {
  ...
  const getSituacaoBadge = (situacao: string) => {
    const variants: Record<string, {
      ...
      Liberado: {
        variant: "default",
        className: "bg-verde-sucesso hover:bg-verde-sucesso/90"
      },
      Impresso: {
        variant: "default",
    ...
    const config = variants[situacao] || {
  ...
  return <div className="flex min-h-screen w-full bg-gradient-services">
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4">
                <div className="space-y-2">
                  ...
                  <Input placeholder="Ex: REQ-2024-001234" value={filters.requisicao} onChange={e => setFilters({
                  ...filters,
                  requisicao: e.target.value
                ...
                <div className="space-y-2">
                  ...
                  <Input placeholder="Buscar por nome..." value={filters.paciente} onChange={e => setFilters({
                  ...filters,
                ...
                <div className="space-y-2">
                  ...
                  <Input type="date" value={filters.data} onChange={e => setFilters({
                  ...filters,
                  data: e.target.value
                })} className="input-modern" />
                </div>
                <div className="space-y-2">
                  ...
                  <Input placeholder="Nome do convênio..." value={filters.convenio} onChange={e => setFilters({
                  ...filters,
                ...
                <div className="space-y-2">
                  ...
                  <Select value={filters.situacao} onValueChange={value => setFilters({
                  ...filters,
                  situacao: value
                })}>
                ...
                <Table>
                  ...
                  <TableBody>
                    {requisicoes.length === 0 ? <TableRow>
                        <TableCell colSpan={9} className="text-center py-12 text-muted-foreground">
                      ...
                      </TableRow> : requisicoes.map(requisicao => <TableRow key={requisicao.id} className="cursor-pointer hover:bg-muted/50 transition-colors" onDoubleClick={() => handleOpenExameSelection(requisicao)}>
                          ...
                          <TableCell className="text-center">
                            <div className="flex items-center justify-center gap-1">
                              {requisicao.examesPendentes > 0 && <Badge variant="destructive" className="text-xs">
                            ...
                            <div className="flex items-center justify-center gap-1">
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Button size="icon" variant="ghost" className="h-8 w-8 hover:bg-primary/10 hover:text-primary" onClick={e => {
                              e.stopPropagation();
                              ...
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Button size="icon" variant="ghost" className="h-8 w-8 hover:bg-verde-sucesso/10 hover:text-verde-sucesso" onClick={e => {
                              e.stopPropagation();
                              ...
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Button size="icon" variant="ghost" className="h-8 w-8 hover:bg-amber-500/10 hover:text-amber-600" onClick={e => {
                              e.stopPropagation();
                              ...
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Button size="icon" variant="ghost" className="h-8 w-8 hover:bg-muted" onClick={e => {
                              e.stopPropagation();
      ...
      {/* Modals */}
      {selectedRequisicao && <>
          <ExameSelectionModal open={showExameSelection} onClose={() => setShowExameSelection(false)} requisicao={selectedRequisicao} />
          <GerarLaudoInternetModal open={showGerarInternet} onClose={() => setShowGerarInternet(false)} requisicao={selectedRequisicao} />
          <GerarLaudoArquivoModal open={showGerarArquivo} onClose={() => setShowGerarArquivo(false)} requisicao={selectedRequisicao} />
          <HistoricoEmissoesModal open={showHistorico} onClose={() => setShowHistorico(false)} requisicao={selectedRequisicao} />
        </>}
    </div>;
};
