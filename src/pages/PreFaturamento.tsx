...
import { ArrowLeft, FileSearch, Plus, FileText, Filter, X, MoreVertical, ChevronDown, ChevronUp, Search, AlertCircle, CheckCircle2, Clock, XCircle, Save, Eye, Trash2, RefreshCcw, Lock, Unlock } from "lucide-react";
...
import { LotePreFaturamentoBadge, getLotePreFaturamentoLegenda, type LotePreFaturamentoStatus } from "@/components/faturamento/StatusBadge";
...
const mockGuias = [{
  ...
  valor: 420.00,
  status: "pendente",
  pendencias: "Código do CID não preenchido",
  selected: true
}, {
  id: 3,
  ...
  valor: 550.00,
  status: "pendente",
  pendencias: "Data da solicitação não informada; Número da carteira inválido",
  selected: false
}, {
  id: 5,
  ...
  valor: 380.00,
  status: "pendente",
  pendencias: "Procedimento não autorizado pelo convênio",
  selected: true
}, {
  id: 8,
...
const PreFaturamento = () => {
  ...
  const handleGuiaSelect = (guiaId: number, checked: boolean) => {
    setGuias(guias.map(g => g.id === guiaId ? {
      ...g,
      selected: checked
    } : g));
  };
        ...
        <main className="flex-1 p-6 overflow-auto">
          ...
          <div className="mb-6 animate-fade-in">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              {viewMode === "list" ? "Conferência de guias antes do envio ao convênio." : "Seleção e conferência de guias do convênio."}
            </p>
          </div>

          {/* Info Card - Collapsible */}
          {viewMode === "list" && <Collapsible open={isInfoExpanded} onOpenChange={setIsInfoExpanded} className="mb-6 animate-fade-in">
          ...
          {/* LIST VIEW */}
          {viewMode === "list" && <div className="space-y-6 animate-fade-in">
              {/* Main Card - Lotes */}
              <div className="bg-card rounded-xl border border-border/50 shadow-sm">
                ...
                <div className="p-5 border-b border-border/50 bg-muted/20">
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Input placeholder="Ex: PF-2024-001" value={filterCodigo} onChange={e => setFilterCodigo(e.target.value)} className="h-9 bg-background" />
                    </div>
                    <div className="space-y-1.5">
                ...
                <div className="p-4 border-b border-border/50">
                  <div className="relative max-w-sm">
                    ...
                    <Input placeholder="Buscar por código da requisição/guia..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-9 h-9 bg-background" />
                  </div>
                </div>
                ...
                <div className="overflow-x-auto">
                  <Table>
                    ...
                    <TableBody>
                      {mockLotes.map(lote => <TableRow key={lote.id} className="cursor-pointer" onClick={() => handleOpenLote(lote)}>
                          ...
                          <TableCell className="text-right font-medium">{lote.total.toLocaleString('pt-BR', {
                        style: 'currency',
                        currency: 'BRL'
                      })}</TableCell>
                          ...
                          <TableCell className="text-center" onClick={e => e.stopPropagation()}>
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" className="bg-background border-border">
                                ...
                                {lote.status === "transicao" && <DropdownMenuItem>
                                    <Lock className="h-4 w-4 mr-2" />
                                    Fechar lote
                                  </DropdownMenuItem>}
                                {lote.status === "fechado" && <DropdownMenuItem>
                ...
                <div className="p-4 border-t border-border/50 bg-muted/20">
                  ...
                  <TooltipProvider>
                    <div className="flex flex-wrap gap-3">
                      {getLotePreFaturamentoLegenda().map(item => <LotePreFaturamentoBadge key={item.status} status={item.status} />)}
                    </div>
                  </TooltipProvider>
                </div>
              </div>
          ...
          {/* DETAIL VIEW */}
          {viewMode === "detail" && selectedLote && <div className="space-y-6 animate-fade-in">
              {/* Lote Status Bar */}
              <div className="bg-card rounded-xl border border-border/50 shadow-sm p-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                  <div className="flex items-center gap-4">
                    ...
                    {selectedLote.status !== "transicao" && selectedLote.dataFechamento && <div className="flex items-center gap-2">
                  ...
                  <div className="flex gap-2">
                    {selectedLote.status === "transicao" ? <Button onClick={handleCloseLote}>
                        <Lock className="h-4 w-4 mr-2" />
                        Fechar Lote
                      </Button> : <Button variant="outline" onClick={handleReopenLote}>
                        <Unlock className="h-4 w-4 mr-2" />
                        Reabrir Lote
                      </Button>}
                  </div>
                ...
                <div className="p-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Input type="date" value={detailDataInicio} onChange={e => setDetailDataInicio(e.target.value)} className="h-9 bg-background" />
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-xs font-medium text-muted-foreground">Data Final do Atendimento</Label>
                      <Input type="date" value={detailDataFim} onChange={e => setDetailDataFim(e.target.value)} className="h-9 bg-background" />
                    </div>
                    <div className="space-y-1.5">
                      <Label className="text-xs font-medium text-muted-foreground">Status das Guias</Label>
                      <div className="flex flex-wrap gap-3 pt-1">
                        <label className="flex items-center gap-1.5 cursor-pointer">
                          <Checkbox checked={detailStatusGuias.includes("aberta")} onCheckedChange={checked => {
                        if (checked) setDetailStatusGuias([...detailStatusGuias, "aberta"]);else setDetailStatusGuias(detailStatusGuias.filter(s => s !== "aberta"));
                      }} />
                          <span className="text-sm">Abertas</span>
                        </label>
                        <label className="flex items-center gap-1.5 cursor-pointer">
                          <Checkbox checked={detailStatusGuias.includes("pendente")} onCheckedChange={checked => {
                        if (checked) setDetailStatusGuias([...detailStatusGuias, "pendente"]);else setDetailStatusGuias(detailStatusGuias.filter(s => s !== "pendente"));
                      }} />
                          <span className="text-sm">Pendentes</span>
                        </label>
                        <label className="flex items-center gap-1.5 cursor-pointer">
                          <Checkbox checked={detailStatusGuias.includes("cancelada")} onCheckedChange={checked => {
                        if (checked) setDetailStatusGuias([...detailStatusGuias, "cancelada"]);else setDetailStatusGuias(detailStatusGuias.filter(s => s !== "cancelada"));
                      }} />
              ...
              <div className="bg-card rounded-xl border border-border/50 shadow-sm">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-5 border-b border-border/50">
                  ...
                  <div className="flex items-center gap-4 text-sm">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-semibold text-verde-ativo">{totalSelecionado.toLocaleString('pt-BR', {
                      style: 'currency',
                      currency: 'BRL'
                    })}</span>
                    </div>
                ...
                {/* Actions Bar */}
                {selectedLote.status === "transicao" && <div className="p-4 border-b border-border/50 bg-muted/20 flex flex-wrap items-center gap-2">
                ...
                <div className="overflow-x-auto">
                  <Table>
                    <TableHeader>
                      <TableRow className="hover:bg-transparent">
                        {selectedLote.status === "transicao" && <TableHead className="w-12">
                            <Checkbox checked={guias.length > 0 && guias.every(g => g.selected)} onCheckedChange={checked => handleSelectAll(!!checked)} />
                          </TableHead>}
                        <TableHead className="font-semibold">Código</TableHead>
                    ...
                    <TableBody>
                      {guias.map(guia => <TableRow key={guia.id} className={cn(guia.status === "pendente" && "bg-vermelho-moderno/5", guia.status === "cancelada" && "bg-muted/50 opacity-60")}>
                          {selectedLote.status === "transicao" && <TableCell>
                              <Checkbox checked={guia.selected} onCheckedChange={checked => handleGuiaSelect(guia.id, !!checked)} disabled={guia.status === "cancelada"} />
                            </TableCell>}
                          ...
                          <TableCell className="text-right font-medium">{guia.valor.toLocaleString('pt-BR', {
                        style: 'currency',
                        currency: 'BRL'
                      })}</TableCell>
                          <TableCell>{getGuiaStatusBadge(guia.status as GuiaStatus)}</TableCell>
                          <TableCell className="max-w-xs">
                            {guia.pendencias ? <div className="flex items-start gap-1.5">
