...
const RelatorioItensPendentes = () => {
  const [dataInicio, setDataInicio] = useState<Date>();
  const [dataFim, setDataFim] = useState<Date>();
  const [unidade, setUnidade] = useState("");
  const [convenio, setConvenio] = useState("");
  const [tipoExame, setTipoExame] = useState("");
  const [numeroRepeticao, setNumeroRepeticao] = useState("");
  ...
  const handleGerarRelatorio = () => {
    if (!dataInicio || !dataFim) {
      toast({
        ...
        description: "Informe a data inicial e final do período de atendimento.",
        variant: "destructive"
      });
      return;
    }
        ...
        <main className="flex-1 p-6 overflow-auto">
          ...
          <div className="mb-6">
            ...
            <p className="text-muted-foreground mt-1">
              Selecione os filtros para gerar o relatório de exames sem coleta ou ainda pendentes.
            </p>
          </div>
            ...
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div className="space-y-2">
                  <Label className="text-sm">Período Atendimento - Início *</Label>
                  <Popover>
                ...
                {/* Data Fim */}
                <div className="space-y-2">
                  <Label className="text-sm">Período Atendimento - Fim *</Label>
                  <Popover>
                    <PopoverTrigger asChild>
                      <Button
                        variant="outline"
                ...
                <div className="space-y-2">
                  ...
                  <Select value={tipoExame} onValueChange={setTipoExame}>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="anatomia">Anatomia Patológica</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                ...
                {/* Número de Repetição */}
                <div className="space-y-2">
                  <Label className="text-sm">Número de Repetição</Label>
                  <Input
                    placeholder="Nº repetição (opcional)"
                    value={numeroRepeticao}
                    onChange={(e) => setNumeroRepeticao(e.target.value)}
                  />
                </div>
              </div>
          ...
          <Dialog open={showModal} onOpenChange={setShowModal}>
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  Itens Pendentes - {dataInicio && format(dataInicio, "dd/MM/yyyy")} a {dataFim && format(dataFim, "dd/MM/yyyy")}
                </DialogDescription>
              </DialogHeader>
              ...
              <div className="py-8 flex flex-col items-center justify-center">
                {isGenerating ? (
                  ...
                  <>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">relatorio_itens_pendentes.pdf</p>
                  </>
                ) : null}
              </div>
...
export default RelatorioItensPendentes;
